...
public:
	...
	class D2ItemTypesTxt *pItemTypesTxt; //0x1270
	...
	class D2ObjectsTxt *pObjectsTxt; //0x1430
	...
	class D2RareAffixTxt *pMagicAffixTxt; //0x19A0
	uint64_t nMagicAffixTxtCount; //0x19A8
	uint64_t nMagicAffixTxtCountEx; //0x19B0
	class D2RareAffixTxt *pMagicSuffix; //0x19B8
	class D2RareAffixTxt *pMagicPrefix; //0x19C0
	class D2RareAffixTxt *pAutoMagic; //0x19C8
	class D2RareAffixTxt *pRareAffixTxt; //0x19D0
	...
	class D2ItemsTxt *pItemTxt; //0x1A10
	uint64_t nItemTxtCount; //0x1A18
	uint64_t nItemTxtCountEx; //0x1A20
	class D2ItemsTxt *pWeaponsTxt; //0x1A28
	class D2ItemsTxt *pArmorTxt; //0x1A30
	class D2ItemsTxt *pMiscTxt; //0x1A38
...
static_assert(sizeof(D2DataTablesStrc) == 0x2188);

class D2CharStatsTxt
{
public:
...
static_assert(sizeof(D2CharStatsTxt) == 0xF0);
...
static_assert(sizeof(D2ItemTypesTxt) == 0xE4);
...
static_assert(sizeof(D2ObjectsTxt) == 0x168);
...
static_assert(sizeof(D2LevelsTxt) == 0x188);
...
public:
	...
	class D2RoomStrc *pRoom; //0x0018
	uint32_t ActNo; //0x0020
	char pad_0024[76]; //0x0024
	class D2DrlgStrc *pDrlg; //0x0070
...
public:
	...
	class D2StatsArrayStrc Stats; //0x0030
}; //Size: 0x0048
...
public:
	...
	class D2StatsArrayStrc ModStats; //0x0098 could be wrong
	char pad_00B0[2584]; //0x00B0
	uint32_t StatFlags[6]; //0x0AC8
	uint32_t StatFlagsEx[6]; //0x0AE0
}; //Size: 0x0AF8
static_assert(sizeof(D2StatListExStrc) == 0xAF8);
...
public:
	...
	class D2PathPointStrc SP1; //0x0010
	class D2PathPointStrc SP2; //0x0014
	class D2PathPointStrc SP3; //0x0018
	char pad_001C[4]; //0x001C
	class D2RoomStrc *pRoom; //0x0020
	class D2RoomStrc *pRoomNext; //0x0028
	...
	class D2PathPointStrc PathPoints[78]; //0x00C8
	uint32_t nSavedStepsCount; //0x0200
	class D2PathPointStrc SavedSteps[7]; //0x0204
	char pad_0220[360]; //0x0220
}; //Size: 0x0388
static_assert(sizeof(D2DynamicPathStrc) == 0x388);

class D2RoomStrc
{
public:
	class D2RoomStrc **pRoomsNear; //0x0000
	char pad_0008[16]; //0x0008
	class D2RoomExStrc *pRoomEx; //0x0018
	char pad_0020[24]; //0x0020
	class D2RoomCollisionGridStrc *pCollisionGrid; //0x0038
	uint32_t dwRoomsNear; //0x0040
	char pad_0044[4]; //0x0044
	class D2DrlgActStrc *pAct; //0x0048
...
public:
	...
	class D2ClientStrc *pClientList; //0x0070
	...
	class D2DrlgActStrc *pAct; //0x00B8
}; //Size: 0x00C0
static_assert(sizeof(D2GameStrc) == 0xC0);
...
static_assert(sizeof(D2ClientStrc) == 0x88);
...
public:
	...
	class D2PresetUnitStrc *pPresetNext; //0x0010
	char pad_0018[8]; //0x0018
	uint32_t Type; //0x0020
	uint32_t PosY; //0x0024
	char pad_0028[8]; //0x0028
}; //Size: 0x0030
static_assert(sizeof(D2PresetUnitStrc) == 0x30);
...
static_assert(sizeof(D2StatStrc) == 0x8);
...
public:
	...
	class D2DrlgActStrc *pAct; //0x0858
	char pad_0860[8]; //0x0860
	class D2DrlgLevelStrc *pLevelFirst; //0x0868
...
public:
	...
	class D2RoomTileStrc *pRoomTiles; //0x0078
	char pad_0080[16]; //0x0080
	class D2DrlgLevelStrc *pLevel; //0x0090
	class D2PresetUnitStrc *pPreset; //0x0098
}; //Size: 0x00A0
static_assert(sizeof(D2RoomExStrc) == 0xA0);
...
class D2RoomTileStrc
{
public:
	class D2RoomExStrc *pRoomEx; //0x0000
	class D2RoomTileStrc *pNext; //0x0008
	uint64_t nEnabled; //0x0010
	uint64_t *Unk0x0018; //0x0018
	class D2LvlWarpTxt *pLvlWarpTxtRecord; //0x0020
	uint64_t *Unk0x0028; //0x0028
}; //Size: 0x0030
static_assert(sizeof(D2RoomTileStrc) == 0x30);

class D2DrlgLevelStrc
{
public:
	char pad_0000[16]; //0x0000
	class D2RoomExStrc *pRoomExFirst; //0x0010
	...
	class D2DrlgLevelStrc *pLevelNext; //0x01B8
...
static_assert(sizeof(D2LvlWarpTxt) == 0x60);

class D2StaticPathStrc
{
public:
	class D2RoomStrc *pRoom; //0x0000
